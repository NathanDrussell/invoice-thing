...
> = ({ label, onClick, icon }) => {
  return (
    <button
      className="flex items-center gap-2 rounded border p-2 text-sm leading-none hover:bg-slate-100 focus:ring-1 active:bg-transparent active:ring-2"
      onClick={onClick}
    >
      {icon}
      {label}
...
export const Modal: React.FC<{
  ...
  return (
    <div className="fixed inset-0 z-[99] flex justify-center bg-black bg-opacity-50 py-8 backdrop-blur-sm">
      <div className="flex h-fit max-h-full w-[56rem] flex-col rounded bg-white p-4 shadow-lg">
